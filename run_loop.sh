...
EOF
    ...
    for iter in $(seq $start_iter $end_iter); do
        ...
        existing_games=$(find "${ITER_DATA}" -maxdepth 1 \( -name "*.bin" -o -name "*.bin.zst" -o -name "*.bin.gz" \) 2>/dev/null | wc -l)
        if [ "$existing_games" -ge "$STAGE_GAMES" ]; then
            ...
            # Compress .bin → .bin.zst to save disk (~100x smaller, fast decompress)
            zstd --rm -q "${ITER_DATA}"/game_*.bin 2>/dev/null || true
