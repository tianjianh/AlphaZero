...
//   * Planes 14-17 (ladder features) are zeroed. KataGo's ladder
//     detector requires a full board recurrence with depth limit; we
//     skip it for now and accept a small strength loss vs. upstream.
//     Ladders only appear under specific tactical conditions; the
//     network still gets liberty / area / history features.
...
void encode_for_katago(const GoGame& game,
    ...
    // ── Planes 14-17: ladder features — zeroed (TODO) ───────
    // A faithful port requires a recursive ladder simulator. Skipping
    // these costs some strength but the network still gets stones,
    // libs, area, and history features.
