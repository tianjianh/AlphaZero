...
docker run --rm \
    ...
    /bin/bash -c '
        ...
        echo "[a733] 1) import ONNX → Acuity IR"
        ...
        # Normalize the bs1/bs4 input shapes — the converter inside the
        # container hard-codes 1,22,9,9#1,19 because that is what kata1
        # exports.  We rely on the BS env var for the actual batch.
        if [ "$BS" -ne 1 ]; then
            # Bs!=1: redo the import with the correct shape string.
            # (Keeping the bs=1 string above as a single source of truth
            # for the common case; bs=4 path overrides.)
            rm -rf wksp
            python3 ${ACUITY_PATH}/pegasus.py import onnx \
                --model           ${ONNX_NAME}.onnx \
                --output-model    ${ONNX_NAME}.json \
                ...
                --outputs         "policy_logits value score_mean score_stdev ownership" \
                --size-with-batch "true#true" >/dev/null 2>&1
            python3 ${ACUITY_PATH}/pegasus.py generate inputmeta \
                --model ${ONNX_NAME}.json --separated-database \
                --input-meta-output ${ONNX_NAME}_inputmeta.yml >/dev/null 2>&1
            python3 ${ACUITY_PATH}/pegasus.py export ovxlib \
                --model              ${ONNX_NAME}.json \
                --model-data         ${ONNX_NAME}.data \
                --dtype              float \
        ...
        mkdir -p "$OUT"
        NB=$(find wksp -name "network_binary.nb" -type f | head -1)
        META=$(find wksp -name "nbg_meta.json"     -type f | head -1)
        if [ -z "$NB" ]; then
            echo "[a733] ERROR: NBG not produced — listing wksp/" >&2
            find wksp -type f | head -30 >&2
            exit 1
        fi
...
ok = (magic == 0x4e4d5056 and ver == 0x1001E and target == 0x1000003B)
print(f"[a733] {p}")
print(f"[a733]   magic   = {magic:#x}    (expect 0x4e4d5056 = 'VPMN')")
print(f"[a733]   version = {ver:#x}      (expect 0x1001e)")
