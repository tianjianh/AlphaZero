...
def convert_fp16(onnx_path: str, rknn_path: str, target: str,
                 ...
                 verbose: bool, auto_hybrid: bool = False,
                 dataset: Optional[str] = None) -> None:
    info = _detect_format(onnx_path)
    inputs, shapes = _resolve_input_shapes(info, batch)
    extra = " auto_hybrid=True" if auto_hybrid else ""
    print(f"[fp16] target={target} input_shapes={shapes}{extra}")
    ...
    try:
        ...
        # auto_hybrid is a toolkit ≥2.3.2 knob; it auto-promotes fp16-overflowing
        # layers to int16 using calibration data.  Documented per RKNN SDK
        # guide §6.3.3 ("solve model overflow issues").  Requires a dataset.
        build_kwargs = {"do_quantization": False}
        if auto_hybrid:
            if not dataset or not os.path.exists(dataset):
                raise SystemExit("--auto-hybrid requires --dataset (calibration manifest)")
...
def main():
    ...
    ap.add_argument("--dataset", default=None,
                    help="Calibration manifest (required for --mode hybrid / "
                         "int8, and for --auto-hybrid). "
                         "Generated by tools/rknn_calibration.py.")
    ap.add_argument("--auto-hybrid", action="store_true",
                    help="(fp16 mode only) Enable rknn-toolkit2's auto-hybrid "
                         "pass: per-layer fp16 overflow detection from "
                         "calibration data, with overflowing layers promoted "
                         "to int16.  Requires toolkit ≥ 2.3.2 and a "
                         "--dataset.  Per Rockchip SDK Guide §6.3.3, this is "
                         "the documented fix for fp16-overflow-on-hardware.  "
                         "May be a no-op when the simulator's overflow "
                         "detector misses the bug — see rknnissue_next.md.")
